...
async function handler(ctx) {
    ...
    const out = cards.map((card) => {
        ...
        const buttonTextLater = `<div style="display:flex"><button style="${style}" onclick="${onclickLater}">听</button>`;
        const buttonTextFav = `<button style="${style}" onclick="${onclickFav}">看</button></div>`;
