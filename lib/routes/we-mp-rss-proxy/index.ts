...
// 专用 parser：通过 customFields 直接把 content:encoded 映射到 item.content，避免二次 xml2js 解析
    ...
    const result = await cache.tryGet(
        ...
        async () => {
            try {
                ...
                // 解析 RSS，content:encoded 已通过 customFields 直接映射到 item.content
        ...
        43200
    ); // 缓存12小时（减少大 XML 解析频率，避免阻塞 Node.js 主线程）
